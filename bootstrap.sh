...
if [ ! -e /etc/init.d/elasticsearch ] ; then
  ...
  wget -q https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-1.4.4.noarch.rpm
  ...
  # curl -XPUT 'http://localhost:9200/_river/mongodb/_meta' -d @mongo_river.json
  # curl -XPUT 'http://localhost:9200/_template/airbnb_location' -d @template.json
fi

if [ ! -e /etc/init.d/kibana ] ; then
  curl -sL https://download.elasticsearch.org/kibana/kibana/kibana-4.0.0-linux-x64.tar.gz | sudo tar zx
  mv kibana-4.0.0-linux-x64 /opt
